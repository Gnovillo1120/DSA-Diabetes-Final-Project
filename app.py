...
if selected == "Diabetes Risk Calculator":
    ...
    if st.button("Calculate Risk"):
        ...
        with col1:
            st.markdown(
                result_card(
                    f"{label_dictionary[linear_regression_prediction]} ({linear_regression_probability:.2%})",
                    "Logistic Regression",
    ...
    try:
        ...
        # Filter out models that might have failed (all 0.5 predictions)
        valid_models = {}
        for model_name, probs in all_predictions.items():
            # Check if predictions are not just default values
        ...
        # Calculate MSE for valid models
        mse_scores = {}
        ...
        st.dataframe(
            display_df.style.apply(highlight_winner, axis=1),
